...
Dated : Aug,29, 2021
Task:
Connects to Server and (half duplex chat app with UDP)
UDP Client Chat App
...
int main( int argc, char *argv[])
{
    ...
    {  
       ...
       fgets(buffer,sizeof(buffer),stdin);
         sendto(sockfd,buffer,sizeof(buffer),MSG_CONFIRM,(struct sockaddr*)&serv_addr,sizeof(serv_addr));
         bzero(buffer,sizeof(buffer));
         int recvbytes= recvfrom(sockfd,buffer,sizeof(buffer),MSG_WAITALL,(struct  sockaddr*)&serv_addr,sizeof(serv_addr));
         buffer[recvbytes] = '\0';
